...
new class extends Component {
    ...
    public function bulkActivate(): void
    {
        ...
        session()->flash('success', 'UoM berhasil diaktifkan.');
    }
    ...
    public function bulkDeactivate(): void
    {
        ...
        $this->selected = []; $this->selectAll = false;
        session()->flash('success', 'UoM berhasil dinonaktifkan.');
    }
    ...
    public function saveUom(): void
    {
        ...
        if ($this->editId) {
            ...
            session()->flash('success', 'UoM berhasil diperbarui.');
        } else {
            Uom::create($data);
            session()->flash('success', 'UoM berhasil ditambahkan.');
        }
    ...
    public function deleteUom(int $id): void
    {
        ...
        session()->flash('success', 'UoM berhasil dihapus.');
    }
...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700">
        <div>
            ...
            <p class="text-[12px] text-slate-400 dark:text-slate-500 mt-0.5">Kelola satuan pengukuran material</p>
        </div>
    ...
    {{-- Stats Cards --}}
    <div class="grid grid-cols-1 sm:grid grid-cols-3 lg:grid grid-cols-3 gap-4">
        ...
        <button wire:click="filterByCard('total')" type="button"
            ...
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-xs text-slate-400 dark:text-slate-500 mb-1">Total UoM</p>
...
<div class="flex flex-wrap items-center gap-2 p-4 border-b border-slate-200 dark:border-slate-700">
    ...
    <div x-data="{ open: false }" class="relative">
        <button type="button" @click="open = !open" @click.outside="open = false"
                class="h-9 px-3 flex items-center gap-2 text-sm border border-slate-200
                       dark:border-slate-600 bg-white dark:bg-slate-700 rounded-lg
                       text-slate-600 dark:text-slate-300 hover:bg-slate-50 transition min-w-36">
            <span class="flex-1 text-left text-xs">
        ...
        <div x-show="open"
             ...
             x-transition:enter-start="opacity-0 scale-95 -translate-y-1"
             x-transition:enter-end="opacity-100 scale-100 translate-y-0"
             class="absolute left-0 top-full mt-1 w-40 bg-white dark:bg-slate-800
                    border border-slate-200 dark:border-slate-700 rounded-xl shadow-xl z-30 p-1">
            @foreach(['' => 'All Category', 'Mass' => 'Mass', 'Length' => 'Length', 'Volume' => 'Volume', 'Quantity' => 'Quantity'] as $val => $label)
            <button type="button"
                    wire:click="$set('categoryFilter', '{{ $val }}')"
                    @click="open = false"
                    class="w-full text-left px-3 py-1.5 text-xs rounded-lg transition
                           {{ $categoryFilter === $val
                               ? 'bg-[#1a3a6e] text-white'
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50">
                        ...
                        @endif
                        @if($columns['category'])
                        <th class="px-3 py-3 text-left text-xs font-semibold text-slate-400 uppercase tracking-wide">Category</th>
                        @endif
                        @if($columns['note'])
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/40 transition-colors group">
                        ...
                        @endif
                        @if($columns['name'])
                        <td class="px-3 py-2.5 font-medium text-[12px] text-slate-800 dark:text-slate-100">{{ $uom->name }}</td>
                        @endif
                        @if($columns['category'])
                        <td class="px-3 py-2.5">
                            @php
                            ...
                            <span class="text-[12px] px-2 py-0.5 rounded-full font-medium
                                         {{ $cat[$uom->category] ?? 'bg-slate-100 text-slate-600' }}">
                                {{ $uom->category }}
                            </span>
                        </td>
                        @endif
                        ...
                        <td class="px-3 py-2.5">
                            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button type="button"
                                        wire:click="deleteUom({{ $uom->id }})"
                                        wire:confirm="Yakin hapus UoM '{{ $uom->name }}'?"
                                        class="w-7 h-7 flex items-center justify-center rounded-lg
...
<div class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 p-0 sm:p-4">
    ...
    <div class="
        w-full sm:max-w-lg bg-white shadow-xl
        ...
        <div class="px-6 py-5 space-y-4">
            ...
            <div class="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
                <div>
                    ...
                    <p class="text-xs text-slate-400">{{ $form_status ? 'UoM aktif' : 'UoM tidak aktif' }}</p>
                </div>
